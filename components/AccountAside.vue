...
<template>
    <aside class="w-[380px]  flex flex-col min-h-[calc(100svh)] fixed py-16  max-[1224px]:py-16 justify-between bg-white items-center max-[822px]:px-0 gap-4 " :class="[
        ...
        <ul class="flex flex-col w-full z-0 max-[822px]:items-center gap-8 text-white px-4 max-[822px]:px-1  max-[822px]:gap-8 text-base" :class="[
            ...
            <li class="w-full flex items-center gap-2 max-[822px]:justify-between font-medium p-4 text-slate-900  max-[822px]:border-none max-[822px]:text-sm cursor-pointer transition-all duration-300" v-for="(item, i) in menuItems" :key="item.icon" :class="[
                {
             ...
             @click="asideStore.setSelectedSection(i)">
                ...
                <Icon :name="item.icon" class="min-h-8 min-w-8 max-[822px]:min-w-6" :class="[{
